...
const ElectricBorder: React.FC<ElectricBorderProps> = ({
  ...
  const updateAnim = () => {
    ...
    if (dyAnims.length >= 2) {
      (dyAnims[0] as SVGAnimateElement).setAttribute('values', `${height}; 0`);
      (dyAnims[1] as SVGAnimateElement).setAttribute('values', `0; -${height}`);
    }

    const dxAnims = Array.from(svg.querySelectorAll('feOffset > animate[attributeName="dx"]'));
    if (dxAnims.length >= 2) {
      (dxAnims[0] as SVGAnimateElement).setAttribute('values', `${width}; 0`);
      (dxAnims[1] as SVGAnimateElement).setAttribute('values', `0; -${width}`);
    }

    const baseDur = 6;
    const dur = Math.max(0.001, baseDur / (speed || 1));
    [...dyAnims, ...dxAnims].forEach(a => (a as SVGAnimateElement).setAttribute('dur', `${dur}s`));
